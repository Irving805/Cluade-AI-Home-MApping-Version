...
// ============= LOWER CABINET UNITS ESTIMATOR =============

// Base lower cabinet units by bedroom count
const LOWER_CABINET_UNITS_BY_BEDROOMS: Record<number, number> = {
  0: 4,   // Studio - compact galley
  1: 5,   // 1 Bed - small kitchen
  2: 7,   // 2 Bed - standard
  3: 10,  // 3 Bed - family kitchen
  4: 13,  // 4 Bed - large kitchen
  5: 16,  // 5 Bed - expansive
  6: 20,  // 6+ Bed - estate kitchen
};

/**
 * Estimate the number of lower/base cabinet units based on home size.
 * Uses bedrooms, sqft, property type, and cabinet override to derive a realistic count.
 *
 * @param bedrooms - Number of bedrooms (0 = Studio, 6+ = 6)
 * @param sqftRange - Square footage range enum
 * @param propertyType - Property type
 * @param cabinetOverride - User override for cabinet count
 */
export function estimateLowerCabinetUnits(params: {
  bedrooms: number;
  sqftRange?: string;
  propertyType?: PropertyCategory | string;
  cabinetOverride?: CabinetSizeOverride;
}): number {
  const { bedrooms, sqftRange, propertyType, cabinetOverride = 'typical' } = params;

  // 1. Base units from bedrooms
  const clampedBeds = Math.min(Math.max(bedrooms, 0), 6);
  const baseUnits = LOWER_CABINET_UNITS_BY_BEDROOMS[clampedBeds] ?? 10;
  ...
  // 5. Calculate estimated units (round to whole number)
  return Math.round(baseUnits * sqftFactor * structureFactor * overrideFactor);
}
